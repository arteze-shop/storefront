...
export const OrderSummary: FC<OrderSummaryProps> = ({ checkout, order, editable, onCheckoutChange }) => {
	...
	return (
		<article>
			...
			<header className="hidden items-center gap-2 bg-secondary/30 px-5 py-4 font-fraunces md:flex">
							...
							{lines.map((line) => (
								<li key={line.id} className="flex gap-4 py-2">
									...
									<figure className="relative shrink-0">
										...
										<div className="h-14 w-14 overflow-hidden rounded-lg border border-border bg-transparent">
											{line.imageUrl ? (
												<Image
													src={line.imageUrl}
													alt={line.imageAlt || line.name}
